...
data <- read_rds(here(data_dir,"cleaned.rds"))
...
first_spr <- d.raw %>% 
  ...
  tally() %>% 
  mutate(pct=n/sum(n)) %>% 
  mutate(source="SPR 1st story") %>% 
  select(source,correct,pct)

all_maze <- data_filt %>% select(subject, num_correct) %>% 
  unique() %>% 
...
maze_bad <- data_filt %>% select(subject, num_correct) %>% 
  unique() %>% 
  left_join(data_error_summ, by=c("subject", "num_correct")) %>% 
  ...
  tally() %>% 
  mutate(pct=n/sum(n)) %>% 
  mutate(source="<80% correct") %>% 
  select(source,correct, pct)

maze_good <- data_filt %>% select(subject, num_correct) %>% 
  unique() %>% 
  left_join(data_error_summ, by=c("subject", "num_correct")) %>% 
  filter(pct_correct>.8) %>% 
  mutate(correct=num_correct) %>% 
...
to_plot <- all_maze %>% union(maze_bad) %>% union(maze_good)  %>% union(all_spr) %>% union(first_spr) %>% write_rds(here("Analysis/models/comp.rds"))
