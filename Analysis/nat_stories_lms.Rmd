...
labelled_anything_goes <- data_anything_goes %>% inner_join(past_word, by=c("word_num", "word", "sentence")) %>%
  filter(word_num>1) %>% 
select(rt, subject, Word_In_Story_Num, word, Story_Num,txl_surp,ngram_surp,grnn_surp, freq,length,
         txl_center, ngram_center,grnn_center,freq_center, length_center, starts_with("past")) %>% 
  mutate(Word_ID=as_factor(str_c(Story_Num, Word_In_Story_Num, sep="_"))) %>% 
  select(-Word_In_Story_Num, -Story_Num) %>% write_rds("non-selective.rds")
```
...
any_grnn <- read_rds("brm_grnn_any_center.rds")
any_txl <- read_rds("brm_txl_any_center.rds")
any_ngram <- read_rds("brm_ngram_any_center.rds")
