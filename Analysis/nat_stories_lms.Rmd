...
ggplot(select_labs, aes(length))+geom_bar()
...
ggsave("../Papers/gam2.pdf", width=3.5, height=2, unit="in")

for_graph_3 <- labelled_post_only %>% 
  select(rt, txl_surp, ngram_surp, grnn_surp, past_txl, past_grnn, past_ngram) %>%
  pivot_longer(cols=`txl_surp`:`past_ngram`,names_to="label", values_to="data") %>% 
  mutate(type=case_when(
         label %in% c("past_ngram", "past_txl", "past_grnn")~"Previous",
...
ggplot(for_graph_3, aes(x=data, y=rt))+
         geom_smooth(method="gam",formula= y~s(x, bs="cr", k=20),color="black")+
         facet_grid(type~model)+
        labs(x="Surprisal (bits)", y="Reaction Time (ms)")

ggsave("../Papers/gam3.pdf", width=3.5, height=2, unit="in")
...
In terms of surprisal analyses, the possible next steps are doing GAM analysis more robustly (with mixed effects, presenting results from the models) and doing DTSR. 

We could also try looking at the specific syntactic features that are in the dataset (the point of natural stories was to have a bunch of idioms and weird syntax); we could maybe analyse those?
