...
We sometimes center the predictors, but don't rescale them. (We center them for the brm models, but don't center surprisal for the GAM model).

The model formula we use is rt ~ surp + length * freq + past_surp + past_length * past_freq, so we're allowing frequency and length to interact and including the same terms for the current and past word. 
...
lm_ngram_model_1 <- lm(rt ~ ngram_center + freq_center * length_center+ past_c_ngram + past_c_freq*past_c_length, data=labelled_pre_error)

lm_txl_model_1 <- lm(rt ~ txl_center + freq_center * length_center+ past_c_txl + past_c_freq*past_c_length, data=labelled_pre_error)

lm_grnn_model_1 <- lm(rt ~ grnn_center + freq_center * length_center+ past_c_grnn + past_c_freq*past_c_length, data=labelled_pre_error)
...
lm_ngram_model_2 <- lm(rt ~ ngram_center + freq_center * length_center+ past_c_ngram + past_c_freq*past_c_length, data=labelled_post_error)

lm_txl_model_2 <- lm(rt ~ txl_center + freq_center * length_center+ past_c_txl + past_c_freq*past_c_length, data=labelled_post_error)

lm_grnn_model_2 <- lm(rt ~ grnn_center + freq_center * length_center+ past_c_grnn + past_c_freq*past_c_length, data=labelled_post_error)
...
We include a by-subject effect for everything. Technically speaking, we do have repeated measures at the word level, but there are 10K or so words and at most a handful of copies of each word, so that's likely to get us not very much at the cost of making the model rather intractable (and so I'm not going to deal with it, at least not right now). 
...
```{r}
 ...
 brm_ngram_model <- brm(rt ~ ngram_center + freq_center * length_center +
                         past_c_ngram + past_c_freq*past_c_length +
                         (ngram_center + freq_center * length_center +
                            past_c_ngram + past_c_freq*past_c_length|subject), data=labelled_pre_error,
                       file="brm_ngram_pre_center", prior=priors, control=list(adapt_delta=.95))

brm_grnn_model <- brm(rt ~ grnn_center + freq_center * length_center +
                         past_c_grnn + past_c_freq*past_c_length +
                         (grnn_center + freq_center * length_center +
                            past_c_grnn + past_c_freq*past_c_length|subject), data=labelled_pre_error,
                       file="brm_grnn_pre_center", prior=priors, control=list(adapt_delta=.95))

brm_txl_model <- brm(rt ~ txl_center + freq_center * length_center +
                         past_c_txl + past_c_freq*past_c_length +
                         (txl_center + freq_center * length_center +
                            past_c_txl + past_c_freq*past_c_length|subject), data=labelled_pre_error,
                       file="brm_txl_pre_center", prior=priors, control=list(adapt_delta=.95))
...
```
```{r}
summary(brm_txl_model)
summary(brm_grnn_model)
summary(brm_ngram_model)
```

```{r}
a <- show_summary(brm_txl_model) %>% mutate(model="TXL") %>% mutate(Term=str_replace(Term,"txl","surp"))
b <- show_summary(brm_grnn_model) %>% mutate(model="GRNN") %>% mutate(Term=str_replace(Term,"grnn","surp"))
c <- show_summary(brm_ngram_model) %>% mutate(model="5-gram") %>% mutate(Term=str_replace(Term,"ngram","surp"))
...
I think there might be ways to have mixed effects for this, but for now we're not. Also, couldn't figure out good ways of getting graphs from the full models, so just fitting the pieces within ggplot for graphs.
...
```{r}
for_graph <- labelled_pre_error %>% 
  select(rt, txl_surp, ngram_surp, grnn_surp, past_txl, past_grnn, past_ngram) %>%
  pivot_longer(cols=`txl_surp`:`past_ngram`,names_to="label", values_to="data") %>% 
  mutate(type=case_when(
         label %in% c("past_ngram", "past_txl", "past_grnn")~"Previous",
         T~"Current"),
         model=case_when(
           label %in% c("txl_surp", "past_txl")~"TXL",
           label %in% c("grnn_surp", "past_grnn")~"GRNN",
           T ~ "5-gram"
         ))

ggplot(for_graph, aes(x=data, y=rt))+
         geom_smooth(method="gam",formula= y~s(x, bs="cr", k=20),color="black")+
         facet_grid(type~model)+
        labs(x="Surprisal (bits)", y="Reaction Time (ms)")

ggsave("../Papers/gam.pdf", width=3.5, height=2, unit="in")
...
```{r plot_gam-3}
for_graph_2 <- labelled_post_error %>% 
  select(rt, txl_surp, ngram_surp, grnn_surp, past_txl, past_grnn, past_ngram) %>%
  pivot_longer(cols=`txl_surp`:`past_ngram`,names_to="label", values_to="data") %>% 
  mutate(type=case_when(
         label %in% c("past_ngram", "past_txl", "past_grnn")~"Previous",
...
ggplot(for_graph_2, aes(x=data, y=rt))+
         geom_smooth(method="gam",formula= y~s(x, bs="cr", k=20),color="black")+
         facet_grid(type~model)+
        labs(x="Surprisal (bits)", y="Reaction Time (ms)")
