...
all_mean_spr <- ngram_mean_data %>% union(grnn_mean_data) %>% union(txl_mean_data) %>% union(gpt_mean_data) 
...
m_ngram <- run_lm(all_mean_spr,"5-gram")
m_grnn <- run_lm(all_mean_spr, "GRNN")
m_txl <- run_lm(all_mean_spr,"TXL")
m_gpt <- run_lm(all_mean_spr,"GPT-2")
