...
Word level correlation between maze and spr
```{r}

summ_spr <- spr %>% group_by(Story_Num, Sentence_Num, word_num) %>% summarize(spr=mean(rt))

summ <- maze %>% group_by(Story_Num, Sentence_Num, word_num) %>% summarize(maze=mean(rt)) %>% left_join(summ_spr)
...
cor(summ$spr,summ$maze, use="complete.obs")

```

sentence level correlation between maze and spr

Note that a lot of this is driven purely by sentence length, which isn't interesting. 
```{r}
sentence <- summ %>% group_by(Story_Num, Sentence_Num) %>% 
...
cor(sentence$spr,sentence$maze, use="complete.obs")
```

Averaging across words at the sentence level removes the sentence-length effect.

```{r}
sentence_avg <- summ %>% group_by(Story_Num, Sentence_Num) %>% 
...
cor(sentence_avg$spr,sentence_avg$maze, use="complete.obs")

```

As a baseline, we can do all the same things on split-halfs of each data set. We split the subjects in half. (Note that this means that each half has the same # of subjects, but may not have the same #s of each story)

Within spr correlations are higher than spr-maze. 

```{r spr-split-half}
set.seed(42)
spr_subs <- spr$WorkerId %>% unique()
length(spr_subs)
#180 participants

spr1 <- sample(spr_subs, 90)

spr_split <- spr %>% 
  mutate(half=ifelse(WorkerId %in% spr1, "spr1","spr2")) %>% 
...
Same things for maze. Note that with fewer subjects, and given only pre-error data, there's a lot more chance of emptiness. I think the weak sentence average correlation is from places where some sentences didn't go as long in one of the halves (b/c low data). 

```{r maze-split-half}
set.seed(42)
maze_subs <- maze$subject %>% unique()
length(maze_subs)
#180 participants

maze1 <- sample(maze_subs, 31)

maze_split <- maze %>% 
  mutate(half=ifelse(subject %in% maze1, "maze1","maze2")) %>% 
