...
write_rds(all_bootstrapped_predictions,"bootstrapped_GAM_predictions.rds")
write_rds(all_surprisal_predictions,"bootstrapped_GAM_surprisal_predictions.rds")
```

```{r example bootstrap results}
...
```{r example non-bootstrap results}
...
gam1 <- read_rds(here("Analysis/bootstrapped_GAM_surprisal_predictions.rds")) %>%  
...
write_rds(all_bootstrapped_predictions_spr,"bootstrapped_spr_GAM_predictions.rds")
...
all_surprisal_predictions_spr <- rbind(rename(bootstrapped_gam_predictions_ngram_spr[[1]],surprisal=surp),
  ...
  select(-term)
write_rds(all_surprisal_predictions_spr,"bootstrapped_spr_GAM_surprisal_predictions.rds")
...
gam1 <- read_rds(here("Analysis/bootstrapped_spr_GAM_surprisal_predictions.rds")) %>%  
  ggplot( aes(x=surprisal, y=rt, ymin=CI_lower, ymax=CI_upper))+
  geom_line()+
  geom_ribbon(alpha=.3)+
  facet_grid(s~model)+
