...
data=read_rds(here("Analysis/models/spr_pre_brms.rds"))
 priors <- c(
      ...
      set_prior("exponential(.1)", class="sd", group="subject", coef="Intercept"),
      set_prior("exponential(.1)", class="sd", group="Word_ID", coef="Intercept"),
...
run_model <- function(model_spec,data,file_name){
  d <- data %>% filter(model==model_spec) %>% 
    mutate(center_rt=scale(rt, center=T, scale=F))
  m1 <- brm(center_rt ~ surp_center * length_center + length_center * freq_center +
                  past_surp_center * past_length_center + past_length_center * past_freq_center + 
                 past2_surp_center * past2_length_center + past2_length_center * past2_freq_center + 
                  past3_surp_center * past3_length_center + past3_length_center * past3_freq_center + 
              (surp_center + length_center + freq_center|subject)+(1|Word_ID),
            data=d,
            file=file_name,
            prior=priors,
            control=list(adapt_delta=.95))
...
```{r diagnose-unhappiness}

sad <- read_rds(here("Analysis/models/gpt_spr.rds"))

summary(sad)

d <- data %>% filter(model=="GPT-2") %>% select(-rt, -Word_ID, -model, -subject)

cor(d) %>%as.tibble() %>%  mutate(across(where(is.numeric), ~round(.x,2))) %>% View()
```
