...
## How to compare localization & effects, (old school freq/length), define surprisal and lang models
TODO!!!
One measure of how localized a method is is to look at how much overall effects on each word stay on that word, versus appearing on downstream words. 
There are a few general effects for how properties of words tend to effect their reading time. 
The classic pair are frequency and length -- longer and less common words take longer to process. @klieglLengthFrequencyPredictability2004
Another measure is predictability, or how expected a word is in context, with more predictable words being read faster @raynerEffectsFrequencyPredictability2004. A word can have a high surprisal for lots of reasons -- it could be low frequency, or semantically unexpected, or the start of a low-frequency syntactic construction, or a word that disambiguates prior words to the less common parse. Many targeted effects of interest contribute to how predictable or unpredictable a word is. If effects of predictability are strong and well-localized, it's a good sign. 
Because predictability is made up of all these components, how do we assess how predictable a word is? One way of doing this is used language models that are trained to predict the next word. Predictability is often measured in terms of bits of surprisal, which is the negative log probability of a word (1 bit of surprisal means a word is expected to occur half the time, 2 bits is 1/4 of the time etc). 
RTs for eye-tracking and SPR show a linear relationship between a words surprisal and RT It's well established for eye-tracking and SPR that RTs are roughly linear in terms of a word's surprisal (negative log probability) [@smithEffectWordPredictability2013; @wilcoxPredictivePowerNeural2020; @goodkindPredictivePowerWord2018; @lukeLimitsLexicalPrediction2016].
 Due to spillover effects, on SPR and eye-tracking, there is also a positive linear relationship between the surprisal of a previous word and the RT on the current word -- this is an indication of lack of localization. 


## why current experiment
TODO!!!
Maze is great, but there hasn't been a study looking at these coarse effects. We're interested in whether there's spillover ,and how similar it is to SPR. It's only used on targeted/constructed materials of independent sentences. 

So we want to do it on some longer natural text and look for these underlying patterns. & see how well participants can read and understand longer texts. 

The Natural Stories corpus [@futrellNaturalStoriesCorpus2020] consists of 10 passages each roughly 1000 words long which are designed to read fluently to native speakers. At the same time, the passages contain copious punctuation, quoted speech, many proper nouns, and low frequency grammatical constructions. Taken together, these properties make this a severe test of our process, as these features make it harder for the language model to choose good distractors and require focus from participants. If participants can succeed at the Maze task on this set of materials, we think they are likely to succeed on a wide variety of naturalistic texts. The corpus is accompanied by binary-choice comprehension questions, 6 per story, which we use to assess comprehension.

We tweak the A-maze task to accomodate these longer passages and then have participants read them and also compare with  @futrellNaturalStoriesCorpus2020 collected self-paced reading time data on this corpus, which we compare the A-maze results to.


Using this tweak, we test A-maze on the passages from the Natural Stories corpus.  Participants were able to read and understand these long passages using A-maze with the error correction paradigm, and their RT profiles show a similar pattern to SPR but with less noise and spillover 
...
```
*mention the whole terminate on errors thing*
...
As described in the Results section, we base all models only on data from participants who choose correctly on at least 80% of the words in the Maze task. 
...
```
To model the relationship between RT and word surprisal, we created a set of predictor variables of frequency, word length, and surprisals from three language models.  For length, we used the length in characters excluding end punctuation. For unigram frequency, we tokenized the training data from @gulordava18 and tallied up instances. We then used the log2 frequency of the expected occurances in 1 billion words as the model predictor, so higher values indicate higher log frequencies. We got per-word surprisals for each of 4 different language models: a Kneser-Ney smoothed 5-gram, GRNN [@gulordava18], Transformer-XL [@daiTransformerXLAttentiveLanguage2019], and GPT-2 [@radfordLanguageModelsAre], using lm-zoo [@gauthierSyntaxGymOnlinePlatform2020]. 

We exclude the first word of every sentence because it has a different distractor, which leaves `r nrow(not_first)` words. Then we exclude words for which we don't have surprisal or frequency information, leaving `r nrow(not_na)` words. We additionally exclude words that were treated as being composed of multiple tokens. This is primarily words that have punctuation, where the punctuation is treated as a separate token. While surprisals should be additive, the summing the surprisals over these parts gives unreasonable responses sometimes. For instance, in one story the word king!' is given a surprisal of 64 by GRNN (context: The other birds gave out one by one and when the eagle saw this he thought, 'What is the use of flying any higher? This victory is in the bag and I am king!'). To avoid these outliers we exclude all words that a model treated as multi-token, leaving `r nrow(not_gpt)` words. (While GPT-2 using byte-pair encoding that can split up words into multiple parts, excluding words it split up only excludes 30 words that were not already excluded by other models.)


For all of these predictors, we consider both the predictor at the current word as well as lagged predictors from the previous word. We excluded outlier RTs that were <100 or >5000 ms (<100 is likely a recording error, >5000 is likely the participant getting distracted). We exclude words where mistakes occurred or which occurred after a mistake in the same sentence. We only analysed words where we had values for all predictors, which means that if the previous word was unknown to a model, this word will be excluded because we're missing values for a lagged predictor. 
...
As a last analysis, we checked which of our surprisal models had the best fit using a nested model comparison shown in Table \@ref(tab:maze-compare). We assess the benefits of adding each model's predictions as a second surprisal source to see which models pick up on information not contained in another model. In terms of log likelihoods, GPT-2 is better than GRNN is better than TXL is better than 5-gram. Model comparisons tell a similar story; GPT-2 provides a lot of additional predictive value over each other model, GRNN provides a lot over 5-gram and TXL and a little complemetary information over GPT-2. TXL provides a lot over 5-gram, and 5-gram provides little over any model. 
```{r maze-compare}
...
Similarly to the model comparison for Maze, we also conducted a model comparison on the SPR data shown in Table \@ref(tab:spr-compare). In terms of log likelihoods, we find that GPT-2 is better than 5-gram is better than GRNN is better than TXL, although differences are small. Model comparisons show that GPT-2 and 5-gram models contain some value over each other model, which is less clear for TXL and GRNN. The relatively good fit of 5-gram models to SPR data compared with neural models matches results from @huSystematicAssessmentSyntactic2020 and @wilcoxPredictivePowerNeural2020. This contrasts with the Maze results, where the 5-gram model has the worst fit and does not provide additional predictive value to the other models. 
