...
shortauthors: "" #Veronica Boyce & Roger Levy
author: 
  - name          : "Anonymous submission" #Veronica Boyce"
...
The Maze task has much less face-validity than eye-tracking or even SPR; repeated forced-choice selections does not look very similar to normal reading. Despite this, @forsterMazeTaskMeasuring2009 report that "At a phenomenological level, participants typically report that they feel as if they are reading the sentence relatively
naturally and that the correct alternative seems to “leap out” at them, so that they do not have to inspect the incorrect
alternative very carefully, if at all." This suggests that the Maze task is not about conscious comparison between two potential continuations and instead relies on the same language processing facilities shared by other methods. Maze is not trying to study the process of normal reading, but instead to get underlying language processing, using an artificial method to slow down and localize language processing. 
...
Another recent variant of the Maze task is interpolated I-maze, which uses a mix of real word distractors (generated via the A-maze process) and non-word distractors [@vaniUsingInterpolatedMaze2021; @wilcoxTargetedAssessmentIncremental2021]. The presence of real word distractors encourages close attention to the sentential context, while non-words can be used as distractors where the word in the sentence is itself ungrammatical or highly unexpected, and/or it is important that the predictability of the distractor in the context is perfectly well-balanced (at zero) across all experimental conditions.
...
Frequency is often found to be a predictor of reading time in SPR and eye-tracking experiments. It is unclear why we find frequency effects on RT when surprisal effects are accounted for. In their discussion, @smith-levy:2008 posit that the role of frequency could diminish or disappear as measurements of surprisal improve (as they have over the past decade). Additionally, @smith-levy:2008 hypothesize that the effects of frequency (in addition to those of surprisal) may be the result of quick, heuristic responses to the words based on the more easily available unigram frequency before more fine-grained context-sensitive surprisal is available. An alternative is that frequency effects don't actually exist, but are an artifact of how RT data is analysed [@shainLargescaleStudyEffects2019]. 
...
We used the Natural Stories corpus [@futrellNaturalStoriesCorpus2020], a set of 10 passages designed to read fluently to native speakers. Each passage is roughly 1000 words long. The passages contain copious punctuation, quoted speech, proper nouns, and low frequency grammatical constructions. The corpus is accompanied by binary-choice comprehension questions, 6 per story, which we used to assess comprehension.
...
# Error-correct Maze: Adapting the Maze paradigm for long passages
...
As a solution, we introduce an *error-correction* variant of Maze shown in Figure \@ref(fig:diagram). When a participant makes an error, they see an error message and must try again to select the correct option, before continuing the sentence as normal. We make error-correction Maze available as an option in a modification of the Ibex Maze implementation introduced in @boyceMazeMadeEasy2020 (link obscured for anonymity) <!--(https://github.com/vboyce/Ibex-with-Maze)-->. The code records both the RT to the first click and also the total RT until the correct answer is selected as separate values.
...
We constructed A-maze distractors for the Natural Stories corpus [@futrellNaturalStoriesCorpus2020] and recruited 100 crowd-sourced participants to each read a story in the Maze paradigm. The materials, data, and analysis code are all available at https://osf.io/gfs87/?view_only=c04ea31c7ea34d9ba84eb106f88b564a <!--https://github.com/vboyce/natural-stories-maze-->. 
...
```{r}
package_list <- c("tidyverse","brms","rstan","bookdown", "rticles", "papaja",
                  "lme4", "mgcv", "mgcViz",
 "tidybayes", "here", "patchwork","cowplot","gridExtra","broom.mixed","tidymv","kableExtra")
...
For linear models, we centered all predictors. We modeled the main effects of surprisal, length, and frequency as well as surprisal-by-length and frequency-by-length interactions.  For the A-maze data, we used maximal mixed effects, including by-subject slopes and a per-word-token random intercept [@barrRandomEffectsStructure2013]. We used weak priors (normal(1000,1000) for intercept, normal(0,500) for beta and sd, and lkj(1) for correlations) and ran models with brm [@R-brms_b]. 

For linear models of the SPR data, we were unable to fit a single model whose random effects structure was maximal with respect to all fixed-effects predictors. We report results for the best (in terms of having maximal random effects structure with respect to fixed effects of primary theoretical interest) single model we could fit: by-subject random intercept, uncorrelated by-subject random slopes for surprisal, length and frequency, and a per-word-token random intercept, fit with lme4 [@R-lme4], as this model specification did not fit reliably in brm.
...
comp_comp <- read_rds(here("Analysis/models/comp.rds")) %>% 
  filter(source %in% c(">80% correct","<80% correct", "SPR 1st story")) 

comp_tally <- read_rds(here("Analysis/models/comp.rds")) %>% group_by(source) %>% filter(correct>4) %>% 
...
## How do A-maze and SPR compare in reliability?
...
# mod1 <- lm(spr ~ maze,both)
# mod2 <- lm(maze ~ spr,both)
# slope2 <- 1/coef(mod2)[2]
# intercept2 <- with(both,mean(spr) - slope2 * mean(maze))
# matr <- with(both,matrix(c(var(maze),cov(spr,maze),cov(spr,maze),var(spr)),2,2))
# slope <- eigen(matr)$vectors[2,1]/eigen(matr)$vectors[1,1]
# intercept <- with(both,mean(spr) - slope * mean(maze))
# ggplot(both, aes(x=maze, y=spr)) + 
#   geom_point(alpha=.5, size=1) + 
#   labs(x="A-Maze", y="SPR") + 
#   #theme(aspect.ratio=1) +
#   coord_fixed(ratio=1) +
#   stat_ellipse(type="norm",segments=501) +
#   stat_ellipse(type="t",color="yellow",segments=501) +
#   #geom_smooth(method="lm") 
#   geom_abline(intercept=coef(mod1)[1],slope=coef(mod1)[2],color="blue") + 
#   geom_abline(intercept=intercept2,slope=slope2,color="red") + 
#   geom_abline(intercept=intercept,slope=slope,color="green")
#   #geom_abline(intercept=0,slope=1)
# 
# slope1 <- 1/coef(mod1)[2]
# intercept1 <- with(both,mean(maze) - slope1 * mean(spr))
# slopeA <- eigen(matr)$vectors[1,1]/eigen(matr)$vectors[2,1]
# interceptA <- with(both,mean(maze) - slopeA * mean(spr))
# ggplot(both, aes(x=spr, y=maze)) + 
#   geom_point(alpha=.5, size=1) + 
#   labs(y="A-Maze", x="SPR") + 
#   theme(aspect.ratio=1) +
#   #coord_fixed(ratio=1) +
...
#   stat_ellipse(type="t",color="yellow",segments=501) +
#   #geom_smooth(method="lm") 
#   geom_abline(intercept=coef(mod2)[1],slope=coef(mod2)[2],color="red") + 
#   geom_abline(intercept=intercept1,slope=slope1,color="blue") + 
#   geom_abline(intercept=interceptA,slope=slopeA,color="green")
#   #geom_abline(intercept=0,slope=1)
```
...
gam1 <- read_rds(here("Analysis/models/bootstrapped_GAM_surprisal_predictions.rds")) %>% mutate(model=factor(model, 
                        levels=c("5-gram",
                                 "GRNN",
                                 "Transformer-XL",
                                 "GPT-2"))) %>%
...
all_mean_data <-  read_rds(here("Analysis/models/meaned_maze.rds")) %>% mutate(model=factor(model, 
                        levels=c("5-gram",
                                 "GRNN",
                                 "TXL",
                                 "GPT-2"))) 
...
dens2 <-   ggplot(all_mean_data, aes(x=value))+
  ...
  theme(axis.text.y = element_blank(), strip.text=element_blank(), 
        axis.ticks.y =element_blank(), axis.title.y=element_blank(), panel.grid=element_blank(), plot.margin = unit(c(0, 0, 0, 0), "cm"))
...
all_mean_spr <-  read_rds(here("Analysis/models/meaned_spr.rds")) %>% mutate(model=factor(model, 
                        levels=c("5-gram",
                                 "GRNN",
                                 "TXL",
                                 "GPT-2")))
...
```{r coeffs-maze, out.width="\\textwidth", fig.width=8, fig.height=3, fig.pos="ht", fig.cap="(ref:coeffs)"}


test_plot <- read_rds(here("Analysis/models/maze_spr_for_graph.rds"))%>% mutate(model=factor(model, 
                        levels=c("5-gram",
                                 "GRNN",
                                 "TXL",
                                 "GPT-2"))) 
...
spr <- read_rds(here("Analysis/models/spr_model_compare.rds"))
```
...
```{r pre-error}
summ <- read_rds(here("Analysis/models/brms_maze_summ.rds")) %>%
  ...
  mutate(Term=c("Intercept", "Surprisal", "Length", "Frequency",
                  ...
                  "Past Length", "Past Freq", "Past Surp x Length", 
                "Past Freq x Length")) %>%
  select(Term, `5-gram`, GRNN, `Transformer-XL`=TXL,`GPT-2`)

knitr::kable(summ, format="latex", position="!h",caption="Predictions from fitted Bayesian regression models. All terms were centered, but not rescaled. Units are in ms. Surprisal is per bit, length per character, and frequency per $log_2$ occurance per billion words. Interval is 2.5th quantile to 97.5th quantile of model draws.")%>%   kable_styling(font_size = 10)
```
...
knitr::kable(summ_spr, format="latex", position="!h",caption="Predictions from fitted regression models for SPR data. All terms were centered, but not rescaled. Units are in ms. Surprisal is per bit, length per character, and frequency per $log_2$ occurance per billion words. Uncertainty interval is +/- 1.97 standard error.") %>%   kable_styling(font_size = 10)
...
```{r gam-interact-table}

gam_interact <- read_rds(here("Analysis/models/gam_gpt_3_summ.rds"))

knitr::kable(gam_interact, format="latex", position="ht",caption="Significance of Generalized Additive Model main effects and two-way interactions among frequency, length, and surprisal for A-Maze reading of the Natural Stories corpus.") %>%   kable_styling(font_size = 10)

```

(ref:gam-grid) Generalized Additive Model main effects and two-way interactions among frequency, length, and surprisal for A-Maze reading of the Natural Stories corpus. Confidence bands do not take into account the uncertainty associated with `mgcv` hyperparameter estimation (@wood:2006GAMs).

```{r gam-grid, out.width="\\textwidth",  fig.pos="ht", fig.cap="(ref:gam-grid)"}
...
The `mgcv` package's implementation of Generalized Additive Models [@wood:2006GAMs] allows linear and nonparametric spline effects of the same continuous predictor to be entered simultaneously into a model. Doing so associates only the nonlinear part of the effect to the spline term, allowing for approximate statistical testing of the linear and non-linear components of the effect respectively. We thus test for whether the effect of surprisal on A-Maze RTs is best described as linear or includes a non-linear component, using the `mgcv` formula:

`rt ~ surprisal + s(surprisal, bs="cr", k=20) + ti(freq, bs="cr") + ti(len, bs="cr") + prev_surp + s(prev_surp, bs="cr", k=20) + ti(prev_freq, bs="cr") + ti(prev_len, bs="cr")`

The results are in Table \@ref(tab:gam-linearity-table). For all but the 5-gram surprisal estimate, there is overwhelming evidence for a linear contribution of current-word surprisal, but little to no evidence for a non-linear contribution. For the 5-gram estimate, there is overwhelming evidence for the linear term, and some evidence for a nonlinearity as well. Consulting Figure \@ref(fig:gam) shows that this nonlinearity takes the form of the surprisal effect dwindling to zero in the sparse tail of high-surprisal words. This nonlinearity is plausibly due to the measurement error (high variance) of using counts to estimate very low multinomial probabilities. Taken together with the 5-gram model's inferior overall fit, we conclude from this analysis that the evidence is quite strong that, like self-paced reading and eye tracking, A-Maze reading of naturalistic texts exhibits a linear effect of surprisal on RTs. 
