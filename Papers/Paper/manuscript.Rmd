...
abstract: |
   Behavioral measures of word-by-word reading time provide experimental evidence to test theories of language processing. A-maze is a recent method for measuring incremental sentence processing that can localize slowdowns related to syntactic ambiguities in individual sentences. We adapted A-maze for use on longer passages and tested it on the Natural Stories corpus. Participants were able to comprehend these longer text passages that they read via the Maze task.  Moreover, the Maze task yielded useable reaction time data with word predictability effects that are linear in the surprisal of the current word, the same pattern found with other tasks. Crucially, Maze reaction times show a tight relationship with properties of the current word, but little spillover of effects from previous words. This superior localization is an advantage of Maze compared with other methods. Overall, we expand the scope of experimental materials, and thus theoretical questions, that can be coupled with the Maze task. 
...
We tweak the A-maze task to accomodate these longer passages and then have participants read the passages in the Maze. We compare our A-maze results with SPR data collected on this corpus by @futrellNaturalStoriesCorpus2020. We find that participants were able to read and understand these long passages using A-maze, and the A-maze RT profiles show similar relationships to surprisal, word length, and word frequency as those seen with SPR, but with less noise and spillover.
...
### Model specification
For generalized additive models, we centered but did not rescale the length and frequency predictors, but left surprisal uncentered for interpretability. We used smooths for the surprisal terms and tensor effects for the frequency by length effects and interactions. To account for by-subject variation, we do bootstrapping ... TODO specs for bootstrapping
...
For model comparison, we took by-item averaged data to aid in fast model fitting.  We included frequency, length, and their interaction to all models. Then we fit models with either 1 or 2 sources of surprisal using lm [@batesFittingLinearMixedeffects2015] and assessed the effect of adding the second surprisal source with an anova. We used predictors for the current and past word and centered all effects. 
...
We first looked at participants overall performance on the Maze task and comrehension questions, in order to see if participants were able to read and understand a story in the Maze task. Many participants were able to complete the Maze task with a high degree of accuracy and answer the comprehension questions correctly. 
...
Given that the Maze task worked, we next consider the relationship between surprisals and Maze RTs. Surprisal, a measure of overall word predictability in context, is linearly related to RT in eye-tracking and SPR. If Maze is measuring the same language processes, we would expect to see a linear relationship in the Maze task. Additionally, surprisal can be used to assess spillover, but comparing the predictive value of current and past word surprisal on current word reading time. 
...
(ref:gam-cap) GAM results for the effect on Maze reaction time (RT) from either current word surprisal (top) or previous word surprisal (bottom). Density of data is shown along the x-axis. For each of the 4 language models used, there is a linear relationship between current word surprisal and RT (at least when there is enough data). The relationship between previous word surprisal and RT is much flatter. TODO these are bootstrapped 95% CI? 
...
The relationships between current and previous word surprisals and RT are shown in Figure \@ref(fig:gam); this is the main effect of either current or previous surprisal. Note that for each of the models, high-surprisal words are rare, with much of the data for words between 0 and 15 bits of surprisal. All of the models show a roughly linear relationship between current word surprisal and RT, especially in the region with more data. All of the models show a flatter relationship between previous word surprisal and RT. This is a sign of localization as the previous word's surprisal is not affecting RT much. 

TODO comment on freq, length effects / non-linearities!

(ref:spr-gam-cap) GAM results for the effect on SPR RT data [@futrellNaturalStoriesCorpus2020] as a function of current word surprisal (top) or the surprisal of an earlier word, up to 3 words back. Density of data is shown along the x-axis. TODO these are bootstrapped 95% CI? 
...
The Maze linear model effects are shown in Table \@ref(tab:pre-error).  we found large effects of surprisal and length, but minimal effects of frequency.  The lack of frequency effects is somewhat surprising, but consistent with @shainLargescaleStudyEffects2019. Notably the coefficients for the lagged terms are small relative to the effects of surprisal and length of the current word, an indication that spillover is limited and effects are strongly localized.  
...
We ran similar models for SPR, although to account for known spillover effects, we consider predictors from the current and 3 previous words. Due to issues fitting models, the details of the models differed (see Methods).  The SPR coefficients are shown in Table \@ref(tab:spr-table). Surprisal, length, and frequency effects are all evident for the current word and surprisal and frequency show effects from the previous word as well. 

Whereas Maze showed surprisal effects in the 10 to 25 ms/bit range and length effects 15-20 ms/character range, SPR effects are about 1/10 this size. This difference in effect is disproportionate to the overall speed of the methods; Maze is 2-3 times as slow as SPR. 

In addition, the relative effects of these different predictors shows somewhat different patterns; in SPR, frequency effects are roughly as big as surprisal effects, but this is not true in Maze. In SPR, length effects exceed surprisal effects, in Maze, this varies depending on the model. 

TODO: is there a way to make the tables more readable / highlight the key rows? Maybe stick the full tables in the appendix and show select versions omitting interactions and 2- 3- past words in the main text? Or maybe it's fine and people can read?

### Model comparison

We use 4 different models for surprisal. This provided robustness to the above analyses, but it also allows us to look at which models have the best fit to human Maze and SPR data. For each data type, we assessed each models fit using log likelihood and R-squared. Then we did a nested model comparison, looking at whether a model with multiple surprisal predictors (ex, GRNN and GPT) had a better fit than a model with only one (ex GRNN alone). This tells us whether a model contains predictive value beyond that contained in another model. As shown in Table \@ref(tab:maze-compare), GPT-2 provides a lot of additional predictive value over each other model, GRNN provides a lot over 5-gram and TXL and a little complementary information over GPT-2. TXL provides a lot over 5-gram, and 5-gram provides little over any model. The single-model measures of log likelihood confirm this hierarchy, as GPT-2 is better than GRNN is better than TXL is better than 5-gram.
```{r maze-compare}
...
As a last analysis, we measured how correlated SPR and Maze are in their reading times; that is, did the methods pick up on the same effects. If so, sentences that take longer to read in one method would also take longer in the other. We calculate the average RT at the sentence level because differences in spillover will effect sentence level RT less than RT for individual words (see Methods for details). The correlation between Maze and SPR is `r round(spr_maze,2)`, compared to `r round(spr_spr,2)` within SPR and `r round(maze_maze,2)` within Maze. See Figure \@ref(fig:scatter) for a visual comparison of overall Maze versus SPR RTs. 

The fact that SPR data is about as correlated with Maze as with another sample of SPR data provides some confirmation that Maze and SPR are measuring the same thing.
...
We took advantage of the pre-existing SPR corpus on Natural Stories to compare the RT profiles between Maze and SPR. While these are not completely comparably as the subject populations differed, we are able to get a sense of the relationship between SPR and Maze results. Maze and SPR pick up on similar features in words, as shown by the correlations between Maze and SPR, and the fact that surprisal and length are predictive of RTs in both. However, the effect sizes of RT and surprisal are much larger is Maze, and the effects are more strongly localized on a single word. 

Some language models are better or worse on predicting human data; however, we find preliminary evidence that how predictive a language model is of human data may depend on what task was used to collect the RTs, with the 5-gram model relatively better at predicting SPR data. Further comparisons between different processing on the same materials could be useful for identifying how task demands influence language processing [ex. @bartekSearchOnlineLocality2011]. 
...
This extreme incrementality makes the Maze task a good target for any question that requires precisely determining the locus of incremental processing difficulty.
