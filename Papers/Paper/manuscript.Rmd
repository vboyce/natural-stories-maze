...
knitr::kable(summ, format="latex", position="t",caption="TODO")
```

```{r post-error}
brm_txl_interact_post <- read_rds(paste0(here(),"/Analysis/brm_txl_interact_post.rds"))
brm_grnn_interact_post <- read_rds(paste0(here(),"/Analysis/brm_grnn_interact_post.rds"))
brm_ngram_interact_post <- read_rds(paste0(here(),"/Analysis/brm_ngram_interact_post.rds"))

d <- show_summary(brm_txl_interact_post) %>% mutate(model="TXL") %>% mutate(Term=str_replace(Term,"txl","surp"))
e <- show_summary(brm_grnn_interact_post) %>% mutate(model="GRNN") %>% mutate(Term=str_replace(Term,"grnn","surp"))
f <- show_summary(brm_ngram_interact_post) %>% mutate(model="5-gram") %>% mutate(Term=str_replace(Term,"ngram","surp"))

summ <- d %>% union(e) %>% union(f) %>% pivot_wider(names_from="model", values_from=c(`Estimate`)) %>% 
   mutate(Term=factor(Term, 
                        levels=c("Intercept", 
                                 "surp_center",
                                 "length_center",
...
As we can see in Table \@ref(tab:pre-error) and Table \@ref(tab:post-error). 
