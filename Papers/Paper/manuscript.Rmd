...
# Methods
We constructed A-maze distractors for the Natural stories corpus (CITE) and recruited 100 MTurkers to each read one the stories in the Maze paradigm. On the basis of previous uses of the Maze paradigm, we estimated that participants would do the task at roughly 1 second per word, or around 17 minutes for a 1000 word passage, so we only gave each participant one passage to read to keep the task manageable. 

## Participants
We recruited 100 participants from Amazon Mechanical Turk, and paid each participant 3.50 dollars for what we estimated was a 20 minute task. In fact, it appears to have taken most attentive participants around 20 minutes to read the story, and thus closer to 25 minutes for the overall study. 
...
Of the `r data %>% select(subject) %>% unique() %>% nrow()` participants who each read one story, we excluded data from those who did not report English as their native language, leaving `r data_filt %>% select(subject) %>% unique() %>% nrow()` participants.  
...
```{r errors, out.height="25%", fig.pos='t', fig.cap="(ref:error-cap)"}
...
Our first concern was whether the paradigm would work at all. To our relief, it did. Some participants were able to complete the Maze task with high accuracy and understand the story while doing so. 

We first wanted to check for participant accuracy, which reflects both how well participants can focus on and navigate the task and whether the auto-generated distractors are of acceptable quality. We calculated the per-word error rate for each participant and compared it against their average reaction time. (To avoid biasing the average if a participant took a pause before returning to the task, RTs greater than 5 seconds were excluded.) As seen in Figure \@ref(fig:errors), there is a cluster of participants who make relatively few errors (marked in green), with some reaching 99% accuracy. This indicates that the distractors were generally appropriate and that some participants can maintain focus on the task for the whole story. These careful participants tend to take around 1 second for each word selection, which is much slower than other paradigms CITE SPR something.

In contrast, we also have a cluster of participants (in red) who we infer sped through the task clicking randomly. This distribution is likely due to the mix of workers on Mturk; as we did not use qualification cutoffs. 

Another important check is whether participants were able to comprehend the story,which we tested using 6 binary-choice comprehension questions after each story. We count how many questions each participant got right, and group them by their performance on the Maze task. As seen in @ref(fig:comp-q), most participants were were accurate on the task also did well on comprehension questions, while participants who were at chance on the Maze task were also at chance on the comprehension questions. Participants usually answered quickly (within 10 seconds), so we do not believe they were looking up the answers on the internet.These are not especially hard questions, and we can't rule out that some participants may have been able to guess the answers without reading the story. Nonetheless, this provides preliminary evidence that people can understand and remember details of stories they read during the Maze task. 

Given that Maze task accuracy and comprehension question accuracy are correlated, but task accuracy is finer-grained and less noisy we use task performance as our exclusion metric. This easy to calculate speed and accuracy comparision makes it easy to see who was paying attention to the task; we exclude the random guessers and only analyse data from participants with at least 80% accuracy (in the gap between high-performers and low-performers). 
