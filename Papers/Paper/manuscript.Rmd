...
[review the terminate thing, and why it was there] The advantage of the Maze task is that it forces incremental processing and automatically excludes inattentive participants by terminating a sentence when a participant makes an error. Thus, only participants who have processed the sentence up that point and are paying attention contribute data at critical regions later in the sentence. However, this causes data loss ecause a participant does not see the rest of the sentence after a mistake, and therefore does not contribute reading time. This data loss problem was relatively minor in traditional G-maze tasks, which had hand-crafted (and thus usually obviously wrong) distractors and attentive, in-person participants. This leads to a low per-word error rate, which does not compound too badly over the individual sentences, but it becomes prohibitive if items grow longer than one or two sentences. TODO: look up what the loss rates were!

[ how this causes problems for 2020 and theoretically] 
However, as @boyceMazeMadeEasy2020 found, this data loss is much more extreme with A-maze materials and crowd-sourced participants. It's unclear to what extent the errors are from participants were guessing randomly or from auto-generated distractors that in fact fit the sentence. Both of these are clearly contributing somewhat. In particular, Boyce et al 2020 found that some of the distractors, especially early in the sentence are problematic, and therefore data-loss is high.

Even if we had attentive participants and good distractors, the Maze paradigm will eventually run into problems with data loss as materials get longer: the errors will build up and few participants will make it to the end. For instance, with a 1% error rate, `r round(.99**15*100)`% would complete each 15-word sentence, but only `r round(.99**50*100)`% of participants would complete each 50 word vignette, and `r round(.99**200*100)`% of 200 word passages.  In order to run longer materials, we need something to do when participants make a mistake, other than terminate the entire item. One option would be skip to the next sentence (rather than next item) when a participant makes a mistake. It could be jarring and confusing to be jumped from the middle of one sentence to start of the next and to be missing some context from the end of the sentence. In addition, this skips over context and potential discourse cues which makes the RTs potentially unreliable. 

To solve this conundrum, we decide to get rid of the source of the problem: the terminating on errors. Instead, we let participants continue on when they make a mistake. To do this, we present them with an error message, and wait until they select the correct option, before continuing the sentence as normal. We added this "error-correction" option to the Ibex Maze implementation (TODO repo link); which can be toggled on or off. The code records both the RT to the first click and also the total RT until the correct answer is selected as separate values. TODO diagram goes here
...
We test this error-correction Maze on the Natural Stories corpus. The Natural Stories corpus [@futrellNaturalStoriesCorpus2020] consists of 10 passages, some fiction and some non-fiction, that are designed to read fluently to native speakers. At the same time, the passages are long, and contain copious punctuation (including quoted speech), many proper nouns, and low frequency grammatical constructions. However, they are intended to read naturally to native English speakers. Taken together, these properties make this a severe test of our process. Long passages test the error-correction process and how well participants can track a story. Punctuation, proper nouns, and infrequent constructions all might stress the NN model generating distractors. We choose this because if participants can succeed at doing the Maze task and understanding it on this set of materials, we think they are likely to on basically any naturalistic text (many of which might be shorter or easier to follow). 

The corpus also comes with binary-choice comprehension questions, 6 per story, which we use to assess comprehension. @futrellNaturalStoriesCorpus2020 ran SPR experiments on the corpus, so we can also compare patterns between Maze and SPR. 


TODO fix this part:
 whether surprisal and other coarse-level processing effects would appear (as a sanity check) -- eye-tracking and SPR show these effects robustly and in relatively well established patterns, and for Maze to be trusted as an alternative, it would need to as well. Maze typically doesn't use comprehension questions because comprehending the local sentence structure is required to do the task. Thus, there isn't much information on how well participants comprehend and remember what they read. We wanted to know if participants understood and internalized the gist of what they were reading with Maze. 
 
This corpus allows us to address a set of sequential questions.
- Firstly, whether this method words at all and whether participants can read stories in the odd paradigm.
- Secondly, that participants can comprehend the stories (as measured by the comprehension questions). 
- Thirdly, that the RT profiles show surprisal and other course-level processing effects.
- Fourthly, although we do not examine it here, the data set can be examined at the targetted syntactic phenomena or compared with NN models. (TODO: unclear whether we should include parts of this). 
...
Participants gave their informed consent, saw instructions, and saw the practice item and practice comprehension questions before reading the main story using the Maze task. After they answered the 6 main comprehension questions, commented on their experience, answered optional demographic questions and were debriefed participants. 
## Participants
We recruited XX participants from Mechanical Turk, and paid each participant YY dollars. 

## Material

We took the corpus and split it up into sentences (this involved some judgment calls due to quoted speech) because the Maze material generation works at the sentence level. We ran the sentences through A-maze generation using TODO foo, bar parameters. Materials are available in TODO: anon repo. We set up the materials in Ibex, using Maze to display things.

We also wrote a short passage to use as practice, with 2 comprehension questions (double check!!). All of these materials are available at TODO
...
We recruited 100 participants, across the 10 stories. (Stories were randomized, but not balanced, so we'll see how balanced the data is.) We estimated that this would take around 20 minutes (given how long past studies had taken) and paid 3.50. In fact, it appears to have taken most attentive participants around 20 minutes to read the story, and thus closer to 25 minutes for the overall study. 
...
Of the `r data %>% select(subject) %>% unique() %>% nrow()` participants who each read one story, we excluded data from those who did not report English as their native language, leaving `r data_filt %>% select(subject) %>% unique() %>% nrow()` participants. After this exclusion, each of the 10 stories was read between `r min(data_stories$n)` and `r max(data_stories$n)` times. 

# Results

## Can people read the stories?

(ref:error-cap) A. Correlation between a participant's accuracy on the Maze task (fraction of words selected correctly) and their average reaction time (in ms). Many participants (marked in green) attained accuracies of >80%; others (in red) appear to be randomly guessing and were excluded from further analysis. B. Performance on comprehension questions. Participants who had >80% task accuracy tended to do well on comprehension questions; those who were at chance on the task were also at chance on comprehension questions. 

```{r errors, out.width="100%", out.height="25%", fig.pos='t', fig.cap="(ref:error-cap)"}
...
The first question of interest is whether participants can do the Maze task on such a long text and whether the auto-generated distractors are clear enough to allow high rates of accuracy. To examine this, we calculate each participants by-word error rate and compare to their average RT (\@ref(fig:errors)). (To avoid biases the average RT estimate if a participant got distracted or stepped away from the computer, RTs greater than 5 seconds were excluded.) A number of participants do the task well, with high per-word accuracies reaching 99%. This indicates that the distractors are mostly not a problem since some people can reliably choose the right option. It also indicates that some participants can maintain focus on the task for the whole story. These participants who are careful tend to take around 1 second per word selection. In contrast, other participants are going much faster and appear to be randomly guessing. We believe this bimodal distribution is a reflection of the mixture of workers on Mturk -- some sped through the task to get paid for as little time as possible, while others took the task seriously. This comparision allows us to easily see who was paying attention to the task; we  only analyse data from participants with at least 80% accuracy.
...
The next question qualitative check is whether participants are able to comprehend the story. We tested this using 6 binary-choice questions after each story. As seen in @ref(fig:comp-q), most participants were were accurate on the task also did well on comprehension questions, while participants who were at chance on the maze task were also at chance on the comprehension questions. One concern is that they comprehension questions may have been too easy, as some of the stories are about factual events (ex. tulip mania) or folk stories. We can't rule out that some participants may have already known some of the answers, but participants usually answered quickly (within 10 seconds), so we do not believe they were looking up the answers on the internet. 

This provides some evidence that people are able to understand and remember details of what they are reading during the Maze task. While the comprehension scores are worse than those from SPR (cite), we must keep in mind that Maze is a much slower task, and therefore participants have more of a delay between the start of the story and answering comprehension questions. We take comprehension question performance as a less reliable indicator of attention that maze task peformance, and so use maze task performance as our exclusion metric. 
